...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('pureMigrationDone_v4')) {
        selectedAPIs = getPureSiteKeys();
        localStorage.setItem('selectedAPIs', JSON.stringify(selectedAPIs));
        localStorage.setItem('pureMigrationDone_v4', 'true');
    }
    ...
    // 首次访问设置完默认值后，刷新豆瓣区域可见性（initDouban 先于此处执行，当时 doubanEnabled 还未写入）
