...
function loadUserTags() {
    try {
        // 旧版本里电影标签误加了"日综"，做一次清理迁移
        const migrated = localStorage.getItem('doubanTagsMigrated_v2') === '1';
        ...
        .catch(error => {
            ...
            container.innerHTML = `
                <div class="col-span-full text-center py-12">
                    <div class="text-red-400 mb-2">❌ 获取豆瓣数据失败，请稍后重试</div>
                    <div class="text-gray-500 text-sm mb-4">可能是豆瓣接口暂时限流，点击下方按钮重试</div>
                    <button id="doubanRetryBtn" class="px-5 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded text-sm transition">
                        重试
                    </button>
                </div>
            `;
            const retryBtn = document.getElementById('doubanRetryBtn');
            if (retryBtn) {
                retryBtn.onclick = () => renderRecommend(tag, pageLimit, pageStart);
            }
        });
}
...
async function fetchDoubanData(url) {
    ...
    // 统一的 fetch 封装，带超时
    async function doFetch(target, init = {}) {
    ...
    // 1) 优先走本站代理（带鉴权，服务器端会自动加浏览器头并对 418 重试）
    const tryLocalProxy = async () => {
        ...
        for (let i = 0; i < 2; i++) {
            try {
                const resp = await doFetch(proxiedUrl);
    ...
    // 2) 备用：公开 CORS 代理（依次尝试，返回原始 JSON）
    const tryPublicProxy = async (proxyBuilder) => {
        const resp = await doFetch(proxyBuilder(url));
        const text = await resp.text();
        try {
            return JSON.parse(text);
        } catch {
            // allorigins /get 格式：{ contents: "..." }
            const data = JSON.parse(text);
            if (data && typeof data.contents === 'string') {
                return JSON.parse(data.contents);
            }
            throw new Error('无法解析备用代理返回的数据');
        }
    };

    const fallbackSources = [
        // allorigins /raw 直接返回原始内容
    ...
    // 依次尝试：本地代理 -> 各公共代理
    try {
        return await tryLocalProxy();
    } catch (localErr) {
        console.warn('本地代理获取豆瓣数据失败，尝试公共代理：', localErr.message);
    }

    let lastErr = null;
    for (const builder of fallbackSources) {
        try {
            return await tryPublicProxy(builder);
        } catch (e) {
            lastErr = e;
            console.warn('公共代理失败，尝试下一个：', e.message);
        }
    }
            ...
            card.innerHTML = `
                <div class="relative w-full aspect-[2/3] overflow-hidden cursor-pointer" onclick="fillAndSearchWithDouban('${safeTitle}')">
                    <img src="${originalCoverUrl}" alt="${safeTitle}" 
                        ...
                        onerror="if(!this.dataset.triedProxy){this.dataset.triedProxy='1';this.src='${proxiedCoverUrl}';this.classList.add('object-contain');}else{this.style.display='none';}"
